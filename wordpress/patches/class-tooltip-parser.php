<?php
/**
 * Page glossary plugin
 *
 * @link       https://capanema.me
 * @since      1.0.0
 *
 * @package    Page_Glossary
 * @subpackage Page_Glossary/public
 */

/**
 * Tooltip parser class, based on WPGH Tooltip. Parse html, adding definition tooltip on golssary words and phrases.
 *
 * @package    Page_Glossary
 * @subpackage Page_Glossary/public
 * @author     Guilherme Capanema de Barros <guilherme@capanema.me>
 */

class PG_Tooltip_Parser
{
     ...
     * @var      string
     */
    protected $tooltip_span_html = '<span class="pg-tooltip" title="%s">\0</span>';
     ...
     * @var      string
     */
    protected $tooltip_link_html = '<a class="pg-tooltip" href="%s" title="%s">%s</a>';
    ...
    public function parse_content($content)
    {
        ...
        foreach ($nodes as $node) {
            ...
            if (is_array($terms)) foreach ($terms as $term) {
                ...
                $pattern = $this->get_term_regex_pattern($term['termo']);
                if (preg_match($pattern, $node->nodeValue) !== 1) {
                    continue; // Not found, continue to next term
                }

                // Add tooltip
                $tooltip    = $this->get_tooltip_html($term['termo'], $term['descricao']);
                $nodeValue  = preg_replace($pattern, $tooltip, $nodeValue, -1);
    ...
    private function get_xpath_query()
    {
        ...
        $disabled_tags = $this->disabled_tags;
    ...
    private function get_tooltip_html($term, $definition)
    {
        ...
        return sprintf($this->tooltip_span_html, $definition);
    ...
    private function clean_definition($definition)
    {
        ...
        $definition = preg_replace('/\$(\d)/', '\\\$$1', $definition);
